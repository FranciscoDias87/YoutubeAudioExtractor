...
class PlaylistWindow(QMainWindow):
    """UI for playlist downloads; download logic is delegated to the controller."""
    ...
    def init_ui(self):
        ...
        info = QLabel("💡 Dica: Certifique-se de que a playlist seja pública para que o download funcione corretamente.")
        info.setStyleSheet("color: #6c757d; font-style: italic; margin: 5px;")
        ...
        self.title_label = QLabel("N/A"); self.title_label.setWordWrap(True)
        self.title_label.setStyleSheet("font-weight: bold;")
        info_layout.addWidget(self.title_label, 0, 1)
        info_layout.addWidget(QLabel("Criador:"), 1, 0)
        self.author_label = QLabel("N/A"); info_layout.addWidget(self.author_label, 1, 1)
        info_layout.addWidget(QLabel("Número de Vídeos:"), 2, 0)
        self.count_label = QLabel("N/A"); info_layout.addWidget(self.count_label, 2, 1)
        info_layout.addWidget(QLabel("Pasta de Destino:"), 3, 0)
        self.folder_label = QLabel("N/A"); self.folder_label.setWordWrap(True)
        ...
        self.format_combo = QComboBox(); self.format_combo.addItems(["mp3", "aac", "wav", "flac", "m4a"])
        config_layout.addWidget(self.format_combo, 0, 1)
        config_layout.addWidget(QLabel("Qualidade (kbps):"), 1, 0)
        self.quality_combo = QComboBox(); self.quality_combo.addItems(["64", "128", "192", "320"]); self.quality_combo.setCurrentText("128")
        config_layout.addWidget(self.quality_combo, 1, 1)
        config_layout.addWidget(QLabel("Diretório Base:"), 2, 0)
        output_layout = QHBoxLayout()
        self.output_label = QLabel(self.output_directory); self.output_label.setWordWrap(True)
        self.output_label.setStyleSheet("background-color: #f0f0f0; padding: 5px; border: 1px solid #ccc;")
        output_layout.addWidget(self.output_label)
        self.browse_button = QPushButton("Procurar"); self.browse_button.clicked.connect(self.browse_output_directory)
        output_layout.addWidget(self.browse_button)
        config_layout.addLayout(output_layout, 2, 1)
        main_layout.addWidget(config_group)

        warning = QLabel("⚠️ Aviso: O download de playlists pode demorar dependendo do número de vídeos. Certifique-se de ter espaço suficiente em disco.")
        warning.setStyleSheet("color: #dc3545; font-weight: bold; background-color: #f8d7da; padding: 10px; border: 1px solid #f5c6cb; border-radius: 5px; margin: 5px;")
        warning.setWordWrap(True); main_layout.addWidget(warning)

        self.download_button = QPushButton("Baixar Playlist Completa")
        self.download_button.clicked.connect(self.start_download); self.download_button.setEnabled(False); self.download_button.setMinimumHeight(45)
        ...
        self.status_log = QTextEdit(); self.status_log.setMaximumHeight(150); self.status_log.setReadOnly(True); self.status_log.setStyleSheet("background-color: #f8f8f8; font-family: monospace;"); main_layout.addWidget(self.status_log)
        self.apply_styles()

    def create_header(self, main_layout):
        header = QHBoxLayout()
        self.back_button = QPushButton("← Voltar ao Menu"); self.back_button.clicked.connect(self.back_to_menu); header.addWidget(self.back_button); header.addStretch()
        title = QLabel("Download de Playlist"); title.setAlignment(Qt.AlignCenter); font = QFont(); font.setPointSize(18); font.setBold(True); title.setFont(font); header.addWidget(title); header.addStretch()
        spacer = QWidget(); spacer.setMinimumWidth(120); header.addWidget(spacer); main_layout.addLayout(header)
    ...
    def process_url(self):
        ...
        if not valid:
            QMessageBox.warning(self, "Aviso", error); return
        self.log_message("Processando playlist...")
        try:
            info = self.controller.inspect(url, single=False)
            self.title_label.setText(info.get("title", "N/A"))
            self.author_label.setText(info.get("uploader", "N/A"))
            entries = [entry for entry in (info.get("entries") or []) if entry]
            self.count_label.setText(str(len(entries)) if entries else str(info.get("playlist_count", "N/A")))
            playlist_title = info.get("title", "Playlist")
            self.folder_label.setText(os.path.join(self.output_directory, FileManager().sanitize_filename(playlist_title)))
            self.download_button.setEnabled(True)
            self.log_message("Playlist processada com sucesso.")
        except Exception as exc:
            self.download_button.setEnabled(False)
            QMessageBox.critical(self, "Erro", f"Não foi possível processar a playlist:\n{exc}")
    ...
    def browse_output_directory(self):
        ...
        if directory:
            self.output_directory = directory; self.output_label.setText(directory)
    ...
    def _download_started(self):
        self.download_button.setEnabled(False); self.process_url_button.setEnabled(False); self.browse_button.setEnabled(False)
        self.progress_bar.setVisible(True); self.progress_bar.setRange(0, 100); self.progress_bar.setValue(0)

    def _download_finished(self):
        self.download_button.setEnabled(bool(self.url_input.text().strip())); self.process_url_button.setEnabled(True); self.browse_button.setEnabled(True)

    def update_progress(self, data):
        status = data.get("status")
        if status == "downloading":
            total = data.get("total_bytes") or data.get("total_bytes_estimate"); downloaded = data.get("downloaded_bytes", 0)
            percent = max(0, min(100, int(downloaded / total * 100))) if total else 0
            self.progress_bar.setValue(percent); self.log_message(f"Baixando: {data.get('_percent_str', f'{percent}%')}")
        elif status == "finished":
            self.progress_bar.setValue(100); self.log_message("Download concluído, processando...")

    def download_finished(self, result):
        self.progress_bar.setValue(100); self.log_message(result.get("message", "Playlist baixada com sucesso!")); QMessageBox.information(self, "Sucesso", result.get("message", "Playlist baixada com sucesso!"))
